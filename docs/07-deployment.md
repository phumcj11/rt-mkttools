...
# Migrations Phase 8
mysql -u root -p rt_mkttools < database/migrations/2026_06_phase8_roles_and_org.sql
mysql -u root -p rt_mkttools < database/migrations/2026_06_phase8_new_modules.sql
```

### 3. โค้ดและ environment
```bash
cd /var/www && git clone https://github.com/phumcj11/rt-mkttools.git rt_mkttools
cd rt_mkttools && cp deploy/.env.production.example .env
# แก้ค่าสำคัญ: DB_DATABASE=rt_mkttools, JWT secrets, OPENAI_API_KEY, ERP_API_KEY
npm install
```
